#!/usr/bin/env pwsh
# Deploy SoundTouch Bridge to TrueNAS via SSH
...
try {
    Write-Host ""
    Write-Host "=== Deploy SoundTouch Bridge to TrueNAS ===" -ForegroundColor Yellow
    Write-Host ""
...
"@
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host "- Or try with sudo: .\deploy-to-truenas.ps1 -TrueNasHost $TrueNasHost -TrueNasUser $TrueNasUser -UseSudo" -ForegroundColor Gray
        Write-Host ""
    }
