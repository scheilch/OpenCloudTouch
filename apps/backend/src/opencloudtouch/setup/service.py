...
class SetupService:
    ...
    async def run_setup(
        ...
        async def update_progress(step: SetupStep, message: str, error: Optional[str] = None):
        ...
        try:
            ...
            await update_progress(SetupStep.CONFIG_MODIFY, "Lese aktuelle Konfiguration...")
            ...
            # Build our BMX URL
            oct_server = self._config.server_url or f"http://{self._config.host}:{self._config.port}"
            new_bmx_url = f"{oct_server}/bmx/registry/v1/services"
            
            # Step 5: Modify config
            await update_progress(SetupStep.CONFIG_MODIFY, f"Setze BMX URL auf {new_bmx_url}...")
            
            # Check if we can write to /opt/Bose (usually read-only)
            # Try creating override in /mnt/nv instead
            result = await client.execute("test -w /opt/Bose/etc && echo 'writable' || echo 'readonly'")
            ...
            if "readonly" in result.output:
                ...
                logger.warning("Config written to /mnt/nv - may need additional override setup")
            else:
    ...
    async def verify_setup(self, ip: str) -> dict:
        ...
        try:
            ...
            check = await client.execute("test -f /mnt/nv/remote_services && echo 'yes'")
