...
class BmxService(BaseModel):
    ...
    authenticationModel: dict[str, Any] = Field(
        default_factory=lambda: {"anonymousAccount": {"autoCreate": True, "enabled": True}}
    )
...
async def resolve_tunein_station(station_id: str) -> BmxPlaybackResponse:
    ...
    try:
        async with httpx.AsyncClient(timeout=10.0) as client:
            ...
            stream_urls = [url.strip() for url in stream_resp.text.splitlines() if url.strip()]
...
async def resolve_stream(request: Request) -> Response:
    ...
    try:
        ...
        # Parse XML request
        root = ElementTree.fromstring(body_str)
