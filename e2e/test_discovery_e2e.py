...
async def test_zeroconf_discovery():
    ...
    print("Discovering via mDNS/Zeroconf...")
    devices = SoundTouchDevice.discovery()
    
    print(f"Zeroconf found {len(devices)} devices")
    for device in devices:
        print(f"  - {device.config.device_name} @ {device.config.device_address}")
...
async def test_api_sync():
    ...
    async with httpx.AsyncClient() as client:
        ...
        response = await client.post("http://hera:7777/api/devices/sync", timeout=30.0)
        ...
        # Check DB
        response = await client.get("http://hera:7777/api/devices")
        response.raise_for_status()
