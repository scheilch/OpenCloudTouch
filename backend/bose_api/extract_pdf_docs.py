...
def extract_endpoints(text: str) -> list:
    ...
    for method, endpoint in matches:
        endpoints.append({"method": method, "endpoint": endpoint.strip("/")})
...
def main():
    ...
    if not pdf_path:
        print("ERROR: PDF not found. Tried:")
        for path in possible_paths:
            print(f"  - {path}")
        return
    ...
    print("2. Compare endpoints with our collected schemas")
    print("3. Identify missing/additional endpoints")
