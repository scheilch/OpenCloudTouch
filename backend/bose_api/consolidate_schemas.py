...
DEVICES = {
    "78": {"name": "ST30_Living Room", "model": "SoundTouch 30 Series III"},
...
"""
    ...
    print(f"    ⚠️  Schemas differ - creating annotated version")
    ...
    header_lines.append('<!-- Note: Detailed diff analysis needed for this endpoint -->')
    header_lines.append('')
...
def main():
    ...
    for endpoint, device_files in sorted(schemas.items()):
        ...
        if devices_with_endpoint != all_devices:
            ...
            print(f"\n  Processing endpoint: {endpoint}")
            print(f"    ⚠️  Device-specific endpoint!")
            print(f"    Available on: {', '.join(DEVICES[d]['name'] for d in sorted(devices_with_endpoint))}")
            print(f"    Missing on: {', '.join(DEVICES[d]['name'] for d in sorted(missing_devices))}")
...
"""
            ...
            if not consolidated_content:
                print(f"    ❌ Failed to consolidate - skipping")
                continue
