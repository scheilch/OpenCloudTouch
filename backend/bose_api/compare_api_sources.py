...
def parse_library_endpoints() -> Dict[str, Dict]:
    ...
    library_endpoints = [
        "addMusicServiceAccount",
        "addStereoPair",
        "addWirelessProfile",
        "addZoneSlave",
        "artistAndPlaylistInfo",
        "audiodspcontrols",
    ...
    for endpoint in library_endpoints:
        endpoints[endpoint] = {"source": "library", "in_soundtouchnodes": True}
...
def generate_report(comparison: Dict):
    ...
    with output_file.open("w", encoding="utf-8") as f:
        ...
        only_official = [
            e
            for e, d in comparison.items()
            if d["in_official"] and not d["in_schemas"] and not d["in_library"]
        ]
        only_schemas = [
            e
            for e, d in comparison.items()
            if not d["in_official"] and d["in_schemas"] and not d["in_library"]
        ]
        only_library = [
            e
            for e, d in comparison.items()
            if not d["in_official"] and not d["in_schemas"] and d["in_library"]
        ]
        in_all_three = [
            e
            for e, d in comparison.items()
            if d["in_official"] and d["in_schemas"] and d["in_library"]
        ]
        missing_from_official = [
            e for e, d in comparison.items() if not d["in_official"]
        ]
        ...
        for endpoint in sorted(comparison.keys()):
            ...
            f.write(
                f"| `/{endpoint}` | {official_mark} | {schemas_mark} | {library_mark} | {notes_str} |\n"
            )
        ...
        if missing_from_official:
            f.write(
                f"**{len(missing_from_official)} endpoints** found in schemas/library but not in official docs:\n\n"
            )
        ...
        if only_official:
            f.write(
                f"**{len(only_official)} endpoints** documented but not found in practice:\n\n"
            )
        ...
        device_specific = [
            e
            for e, d in comparison.items()
            if d.get("schema_info", {}).get("device_specific")
        ]
        if device_specific:
            f.write(
                f"**{len(device_specific)} endpoints** only available on specific models:\n\n"
            )
