...
**Beispiel**: `curl http://192.168.178.78:8090/info`
...
|-------|--------|----|-----------|-----------
| ST30_Wohnzimmer | SoundTouch 30 Series III | 192.168.178.78 | ...20C6 | 28.0.3.46454 |
| ST10_Kueche | SoundTouch 10 | 192.168.178.79 | ...AE25 | 28.0.3.46454 |
| ST300_TV | SoundTouch 300 | 192.168.178.83 | ...1111 | 28.0.3.46454 |
...
**Request**:
```bash
curl http://192.168.178.78:8090/info
```

**Response** (ST30):
```xml
<info deviceID="40EF11B120C6">
  <name>Wohnzimmer</name>
  <type>SoundTouch 30 Series III</type>
  <margeAccountUUID>40EF11B120C6</margeAccountUUID>
  <components>
    <component>
  ...
  <networkInfo type="SCM">
    ...
    <ipAddress>192.168.178.78</ipAddress>
  </networkInfo>
  <networkInfo type="SMSC">
    <macAddress>REDACTED</macAddress>
    <ipAddress>192.168.178.253</ipAddress>
    <type>SMSC</type>
  </networkInfo>
...
**Request**:
```bash
curl http://192.168.178.78:8090/nowPlaying
```
...
**GET Request**:
```bash
curl http://192.168.178.78:8090/volume
```

**Response**:
```xml
<volume deviceID="40EF11B120C6">
...
```bash
curl -X POST http://192.168.178.78:8090/volume \
...
**POST Request** (Play):
```bash
curl -X POST http://192.168.178.78:8090/key \
  -H "Content-Type: application/xml" \
...
**Response** (Master):
```xml
<zone master="40EF11B120C6" senderIPAddress="192.168.178.78">
  <member ipaddress="192.168.178.79">40EF11B1AE25</member>
  <member ipaddress="192.168.178.83">40EF11B11111</member>
</zone>
```

**Response** (Standalone):
```xml
<zone master="40EF11B120C6"/>
```
...
**Beispiele**:
- `device_78_info.xml` - Device Info von ST30 (192.168.178.78)
- `device_79_nowPlaying.xml` - Now Playing von ST10 (192.168.178.79)
...
device = SoundTouchDevice("192.168.178.78")
print(device.Host)           # "192.168.178.78"
print(device.Port)           # 8090
print(device.DeviceId)       # "40EF11B120C6"
print(device.DeviceName)     # "Wohnzimmer"
