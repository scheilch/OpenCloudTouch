...
DEVICES = {
    "ST30_Wohnzimmer": {"ip": "192.168.178.78", "id": "78", "model": "SoundTouch 30"},
    "ST10_Kueche": {"ip": "192.168.178.79", "id": "79", "model": "SoundTouch 10"},
    "ST300_TV": {"ip": "192.168.178.83", "id": "83", "model": "SoundTouch 300"},
}
...
def print_analysis(categorized, device_endpoints):
    ...
    print(f"Total unique endpoints: {len(categorized['common']) + sum(len(eps) for eps in categorized['model_specific'].values())}")
...
def generate_fetch_script(all_endpoints, output_file: Path):
    ...
    for name, config in DEVICES.items():
        script_lines.append(f"    @{{Name='{name}'; IP='{config['ip']}'; ID='{config['id']}'; Model='{config['model']}'}}"
    ...
    read_only_endpoints = sorted([
        ...
        if not any(write_word in ep.lower() for write_word in [
            'set', 'add', 'remove', 'store', 'enter', 'clear', 'select', 
            'pair', 'cancel', 'update', 'factory', 'push', 'play', 'standby',
            'abort', 'start', 'check', 'perform', 'user', 'critical'
        ])
    ])
    
    for i, ep in enumerate(read_only_endpoints):
        script_lines.append(f"    '{ep}'" + ("," if i < len(read_only_endpoints) - 1 else ""))
    
    script_lines.append(")")
    script_lines.append("")
    script_lines.append("Write-Host 'Fetching API schemas from all devices...' -ForegroundColor Cyan")
    script_lines.append("Write-Host")
    script_lines.append("")
    script_lines.append("foreach ($device in $devices) {")
    script_lines.append("    Write-Host \"Device: $($device.Model) ($($device.Name))\" -ForegroundColor Yellow")
    script_lines.append("    ")
    script_lines.append("    foreach ($endpoint in $readOnlyEndpoints) {")
    script_lines.append("        $url = \"http://$($device.IP):8090$endpoint\"")
    script_lines.append("        $filename = \"device_$($device.ID)$($endpoint.Replace('/', '_')).xml\"")
    script_lines.append("        ")
    script_lines.append("        Write-Host \"  Fetching: $endpoint\" -NoNewline")
    script_lines.append("        ")
    script_lines.append("        try {")
    script_lines.append("            $response = Invoke-WebRequest -UseBasicParsing -Uri $url -TimeoutSec 5")
    script_lines.append("            $response.Content | Out-File -Encoding UTF8 \"backend\\bose_api\\$filename\"")
    ...
    print(f"  Write endpoints skipped (safety): {len(all_endpoints) - len(read_only_endpoints)}")
...
if __name__ == "__main__":
    ...
    print("NEXT STEPS:")
    print("=" * 80)
    print(f"1. Run: .\\fetch-api-schemas.ps1")
    print(f"2. Review downloaded schemas in backend/bose_api/")
    print(f"3. Create consolidated API documentation")
