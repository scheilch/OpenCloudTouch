...
def print_capabilities(caps: DeviceCapabilities):
    ...
    print(
        f"  • Audio Level Control: {'✅' if caps.has_audio_product_level_control else '❌'}"
    )
    print(
        f"  • Tone Controls:       {'✅' if caps.has_audio_product_tone_control else '❌'}"
    )
    ...
    hdmi_endpoints = [
        e for e in caps.supported_endpoints if "hdmi" in e.lower() or "cec" in e.lower()
    ]
    audio_endpoints = [
        e
        for e in caps.supported_endpoints
        if "bass" in e.lower() or "audio" in e.lower() or "tone" in e.lower()
    ]
    network_endpoints = [
        e
        for e in caps.supported_endpoints
        if "zone" in e.lower() or "bluetooth" in e.lower()
    ]
...
async def demo_mock_mode():
    ...
    info_st30.DeviceId = "40EF11B120C6"
    ...
    info_st300.DeviceId = "40EF11B11111"
    ...
    assert (
        capabilities_st300.has_audio_product_level_control
    ), "ST300 should have level controls"
...
async def demo_real_device(device_ip: str):
    ...
    try:
        ...
        hdmi_result = await safe_api_call(
            client, SoundTouchNodes.productcechdmicontrol, "HDMI Control"
        )

        if hdmi_result is None:
            print(
                "  ⚠️  HDMI Control endpoint not supported (404/401) - gracefully handled"
            )
        else:
