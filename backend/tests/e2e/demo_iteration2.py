...
class MockRadioBrowserAPI:
    ...
    async def search(self, name: str = None, country: str = None, tag: str = None, limit: int = 10) -> List[Dict[str, Any]]:
...
async def demo_radiobrowser_search(api, use_real: bool):
    ...
    try:
        ...
        for station in stations[:3]:  # Show first 3
            print(f"  - {station['name']} ({station.get('country', 'Unknown')}) - {station.get('codec', '?')}/{station.get('bitrate', '?')}kbps")
...
async def demo_station_detail(api, use_real: bool):
    ...
    try:
        ...
        print(f"  Codec: {detail.get('codec', 'Unknown')} @ {detail.get('bitrate', '?')} kbps")
        print(f"  Country: {detail.get('country', 'Unknown')} ({detail.get('countrycode', '??')})")
...
async def demo_response_format_validation(api, use_real: bool):
    ...
    try:
        ...
        optional_fields = ["url_resolved", "homepage", "favicon", "tags", "bitrate", "votes"]
...
async def demo_backend_api_integration():
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        ...
        try:
            response = await client.get(
                f"{base_url}/api/radio/search",
                params={"q": "relax", "limit": 5}
            )
            response.raise_for_status()
...
async def main():
    """Main entry point."""
    parser = argparse.ArgumentParser(description="E2E Demo for Iteration 2: RadioBrowser API")
    parser.add_argument("--real", action="store_true", help="Use real RadioBrowser API instead of mock")
